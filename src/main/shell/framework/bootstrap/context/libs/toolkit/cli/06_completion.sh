...
__radp_cli_zsh_gen_args_completion() {
  ...
  while IFS= read -r opt_line; do
    ...
    if [[ -n "$opt_spec" ]]; then
      ...
      if [[ "${opt_info[has_value]}" == "true" ]]; then
        ...
        if complete_func=$(radp_cli_get_complete_func "${opt_info[long]}" "${meta[completes]}" 2>/dev/null); then
          # 动态补全
          echo "${pad}        ${opt_spec}[${desc}]:${opt_info[value_name]}:->opt_${opt_info[long]//-/_}' \\"
        else
  ...
  if [[ "$has_dynamic" != "true" ]]; then
    while IFS= read -r arg_line; do
      ...
      local -A arg_info=()
      radp_cli_parse_arg_spec "$arg_line" arg_info
      if radp_cli_get_complete_func "${arg_info[name]}" "${meta[completes]}" &>/dev/null; then
        has_dynamic=true
        break
      fi
    done <<<"${meta[args]}"
  ...
  if [[ "$has_dynamic" == "true" ]]; then
    ...
    while IFS= read -r opt_line; do
      ...
      if [[ "${opt_info[has_value]}" == "true" && -n "${opt_info[long]}" ]]; then
        local complete_func
        if complete_func=$(radp_cli_get_complete_func "${opt_info[long]}" "${meta[completes]}" 2>/dev/null); then
          local state_name="opt_${opt_info[long]//-/_}"
          echo "${pad}        ${state_name})"
          echo "${pad}            local completions"
          echo "${pad}            completions=(\$($complete_func 2>/dev/null))"
          echo "${pad}            _describe '${opt_info[value_name]}' completions"
          echo "${pad}            ;;"
        fi
      fi
    done <<<"${meta[options]}"
    ...
    while IFS= read -r arg_line; do
      ...
      local complete_func
      if complete_func=$(radp_cli_get_complete_func "${arg_info[name]}" "${meta[completes]}" 2>/dev/null); then
        local state_name="arg_${arg_info[name]}"
        echo "${pad}        ${state_name})"
        echo "${pad}            local completions"
        echo "${pad}            completions=(\$($complete_func 2>/dev/null))"
        echo "${pad}            _describe '${arg_info[name]}' completions"
        echo "${pad}            ;;"
      fi
    done <<<"${meta[args]}"
